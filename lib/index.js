...
var aTable = function (_aTemplate) {
  ...
  _createClass(aTable, [{
      ...
      if (pastedData) {
        ...
        if (tableHtml && tableHtml[0]) {
          ...
          if (newRow && newRow.length) {
            e.preventDefault();
            var cellId = this.e.target.parentNode.getAttribute('data-cell-id');
    ...
    value: function insertTable(table, pos) {
      ...
      this.update();
      var index = this.getCellIndexByPos(pos.x, pos.y);

      index.row += copiedLength.y - 1;
      index.col += copiedLength.x - 1;
      var destIndex = this.getCellIndexByPos(index.col, index.row);
      ...
      points.forEach(function (point) {
        if (_this3.hitTest(bound, point)) {
          var _index = _this3.getCellIndexByPos(point.x, point.y);
